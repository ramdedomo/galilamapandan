...
class GalilamapandanDB extends Controller {
     ...
     function addItem(Request $request){
          ...
          if($request->design == 3){
               ...
               if(!empty($request->header) || !empty($request->buttonname) || !empty($request->url) || !empty($request->description)){
                    return back()->with('fail','Youve Selected Image Only!');
               }else{
     ...
     function updateMember(Request $request, $id){
          ...
          if(empty($request->file)){
               ...
               ->where('member_id', $id)
               ->update(['member_name' => $request->memberName, 'member_position' => $request->memberPosition, 'member_order' => $request->memberOrder]);
     ...
     function admin_about_theteam() {
          ...
          return view('admin.admin_about_theteam', ['desc'=>$desc, 'desc2'=>$desc2, 'socials'=>$socials]);
     }
     ...
     function theteam_desc_view(){
          ...
          return view('galila_theteam', ['desc'=>$desc, 'desc4'=>$desc4, 'socials'=>$socials]);
    ...
    function admin_campaigns_campaign_desc_view($id, $id1){
          ...
          if(!$desc->isEmpty()){
               return view('admin.admin_advocacy_campaigns_campaign', ['desc'=>$desc], ['desc2'=>$desc, 'socials'=>$socials]);
          }else{
               return abort(404);
          }
   ...
   public function campaigns_campaign_desc_view($id, $id1) {
     ...
     if(!$desc->isEmpty()){
          return view('galila_advocacy_campaigns_campaign', ['desc'=>$desc], ['desc2'=>$desc, 'socials'=>$socials]);
     }else{
          return abort(404);
     }
